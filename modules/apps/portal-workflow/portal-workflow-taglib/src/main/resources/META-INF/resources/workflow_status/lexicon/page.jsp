...
<span class="taglib-workflow-status">
	...
	<c:if test="<%= Validator.isNotNull(workflowStatusTaglibHelper.getVersion()) %>">
		<span class="mr-2 workflow-version">
			...
			<span class="workflow-value"><%= workflowStatusTaglibHelper.getVersion() %></span>
		</span>
	</c:if>
	...
	<span class="workflow-status">
		...
		<span class="label label-<%= WorkflowConstants.getStatusStyle(workflowStatusTaglibHelper.getStatus()) %> status workflow-status-<%= WorkflowConstants.getStatusLabel(workflowStatusTaglibHelper.getStatus()) %> <%= WorkflowConstants.getStatusCssClass(workflowStatusTaglibHelper.getStatus()) %> workflow-value">
